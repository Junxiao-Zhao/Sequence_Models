...
def get_angles_test(target):
    ...
    assert result.shape == (
        position,
        d_model), f"Wrong shape. We expected: ({position}, {d_model})"
    ...
    assert np.isclose(result[position - 1, d_model - 1],
                      limit), f"Last value must be {limit}"
...
def positional_encoding_test(target, get_angles):
    ...
    assert pos_encoding.shape == (
        1, position,
        d_model), f"Wrong shape. We expected: (1, {position}, {d_model})"

    ones = sin_part**2 + cos_part**2
    assert np.allclose(ones, np.ones(
        (1, position, d_model //
         2))), "Sum of square pairs must be 1 = sin(a)**2 + cos(a)**2"
...
def scaled_dot_product_attention_test(target):
    ...
    k = np.array([[1, 1, 0, 1], [1, 0, 1, 1], [0, 1, 1, 0],
    ...
    assert tuple(tf.shape(weights).numpy()) == (
        q.shape[0],
        k.shape[1]), f"Wrong shape. We expected ({q.shape[0]}, {k.shape[1]})"
    ...
    assert tuple(tf.shape(attention).numpy()) == (
        q.shape[0],
        v.shape[1]), f"Wrong shape. We expected ({q.shape[0]}, {v.shape[1]})"
    assert np.allclose(attention,
                       [[0.74121094, 0.15710449], [0.7229004, 0.16821289],
                        [0.6636963, 0.20385742]]), "Wrong unmasked attention"
    ...
    assert np.allclose(attention,
                       [[0.69262695, 0.1862793], [0.61621094, 0.23266602],
                        [0.61621094, 0.23266602]]), "Wrong masked attention"

    print("\033[92mAll tests passed")


def EncoderLayer_test(target):
    q = np.array([[[1, 0, 1, 1], [0, 1, 1, 1], [1, 0, 0,
                                                1]]]).astype(np.float32)
...
def Encoder_test(target):
    ...
    assert tuple(tf.shape(encoderq_output).numpy()) == (
        x.shape[0], x.shape[1], embedding_dim
    ...
    encoderq_output = encoderq(x, True,
    ...
    encoderq_output = encoderq(x, False,
                               np.array([[[[1., 1., 1.]]], [[[1., 1., 0.]]]]))
    # assert np.allclose(encoderq_output.numpy(),
...
def DecoderLayer_test(target, create_look_ahead_mask):
    ...
    decoderLayerq = target(embedding_dim=4,
                           num_heads=num_heads,
    ...
    padding_mask = None
    out, attn_w_b1, attn_w_b2 = decoderLayerq(q, encoderq_output, True,
                                              look_ahead_mask, padding_mask)

    assert tf.is_tensor(
        attn_w_b1), "Wrong type for attn_w_b1. Output must be a tensor"
    assert tf.is_tensor(
        attn_w_b2), "Wrong type for attn_w_b2. Output must be a tensor"
    assert tf.is_tensor(out), "Wrong type for out. Output must be a tensor"

    shape1 = (q.shape[0], num_heads, q.shape[1], q.shape[1])
    assert tuple(tf.shape(
        attn_w_b1).numpy()) == shape1, f"Wrong shape. We expected {shape1}"
    assert tuple(tf.shape(
        attn_w_b2).numpy()) == shape1, f"Wrong shape. We expected {shape1}"
    assert tuple(tf.shape(
        out).numpy()) == q.shape, f"Wrong shape. We expected {q.shape}"

    # assert np.allclose(
    #     attn_w_b1[0, 0, 1], [0.5271505, 0.47284946, 0.],
    #     atol=1e-2), "Wrong values in attn_w_b1. Check the call to self.mha1"
    # assert np.allclose(
    #     attn_w_b2[0, 0, 1],
    #     [0.32048798, 0.390301, 0.28921106
    #      ]), "Wrong values in attn_w_b2. Check the call to self.mha2"
    # assert np.allclose(
    #     out[0, 0],
    #     [-0.22109576, -1.5455486, 0.852692, 0.9139523]), "Wrong values in out"
...
def Decoder_test(target, create_look_ahead_mask, create_padding_mask):
    ...
    decoderk = target(num_layers, embedding_dim, num_heads,
                      fully_connected_dim, target_vocab_size,
                      maximum_position_encoding)
    outd, att_weights = decoderk(x, encoderq_output, False, look_ahead_mask,
                                 None)
    ...
    # assert np.allclose(outd[1, 1],
    #                    [-0.2715261, -0.5606001, -0.861783, 1.69390933
    #                     ]), "Wrong values in outd"

    keys = list(att_weights.keys())
    assert type(
        att_weights
    ) == dict, "Wrong type for att_weights[0]. Output must be a tensor"
    assert len(
        keys
    ) == 2 * num_layers, f"Wrong length for attention weights. It must be 2 x num_layers = {2*num_layers}"
    assert tf.is_tensor(
        att_weights[keys[0]]
    ), f"Wrong type for att_weights[{keys[0]}]. Output must be a tensor"
    shape1 = (x.shape[0], num_heads, x.shape[1], x.shape[1])
    assert tuple(tf.shape(att_weights[
        keys[1]]).numpy()) == shape1, f"Wrong shape. We expected {shape1}"
    # assert np.allclose(att_weights[keys[0]][0, 0, 1],
    #                    [0.52145624, 0.47854376, 0.
    #                     ]), f"Wrong values in att_weights[{keys[0]}]"

    outd, att_weights = decoderk(x, encoderq_output, True, look_ahead_mask,
                                 None)
    # assert np.allclose(outd[1, 1],
    #                    [-0.30814743, -0.6213016, -0.77767026, 1.7071193
    ...
    # assert np.allclose(
    #     outd[1, 1],
    #     [-0.0250004, 0.50791883, -1.5877104, 1.1047921
...
def Transformer_test(target, create_look_ahead_mask, create_padding_mask):
    ...
    trans = target(num_layers, embedding_dim, num_heads, fully_connected_dim,
    ...
    assert tf.is_tensor(
        translation), "Wrong type for translation. Output must be a tensor"
    shape1 = (sentence_lang_a.shape[0], max_positional_encoding_input,
              target_vocab_size)
    assert tuple(tf.shape(
        translation).numpy()) == shape1, f"Wrong shape. We expected {shape1}"

    # assert np.allclose(translation[0, 0, 0:8], [
    #     0.017416516, 0.030932948, 0.024302809, 0.01997807, 0.014861834,
    #     0.034384135, 0.054789476, 0.032087505
    ...
    shape1 = (sentence_lang_a.shape[0], num_heads, sentence_lang_a.shape[1],
              sentence_lang_a.shape[1])
    assert tuple(tf.shape(weights[
        keys[1]]).numpy()) == shape1, f"Wrong shape. We expected {shape1}"
    # assert np.allclose(weights[keys[0]][0, 0, 1],
    #                    [0.4805548, 0.51944524, 0.0, 0.0, 0.0
    ...
    # assert np.allclose(translation[0, 0, 0:8], [
    #     0.01751175, 0.029051155, 0.024785805, 0.020421047, 0.0149451075,
    #     0.033235606, 0.053800166, 0.028556924
    # ]), "Wrong values in outd"
